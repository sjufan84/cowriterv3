...
import { cloneVocals, testCloneVocalsStream } from '../cloneHelpers/cloneHelperFunctions';
...
type TestClonedVocalsResponse = {
  audioString: string;
  transcriptionText: string;
}

{/* Open the modal using document.getElementById('ID').showModal() method */}
export default function TestVocalCloneModal({ onCloneSuccess }: VocalCloneModalProps) {
  const [currentRecordedVocals, setCurrentRecordedVocals] = useState<Blob | null>(null);
  const [isCloning, setIsCloning] = useState<boolean>(false);
  const [currentClonedVocals, setCurrentClonedVocals] = useState<string | null>(null);
  
  useEffect(() => {
    if (currentClonedVocals) {
      new Audio(currentClonedVocals).play();
    }
  }
  , [currentClonedVocals]);
  ...
  const handleCloneVocals = async () => {
    ...
    setIsCloning(true);
    try {
      const clonedVocalsResponse = await testCloneVocalsStream(currentRecordedVocals) as TestClonedVocalsResponse;
      console.log('Cloned vocals response:', clonedVocalsResponse)
      const transcriptionText = clonedVocalsResponse.transcriptionText;
      const clonedVocals = clonedVocalsResponse.audioString;
      console.log('Cloned vocals:', clonedVocals)
      setCurrentClonedVocals(clonedVocals);
      onCloneSuccess(clonedVocals, transcriptionText);
      setIsCloning(false);
      if (!clonedVocals) {
        alert('Error cloning vocals')
        return
      }
      // onCloneSuccess(clonedVocals, transcriptionText);
      // Close the modal
        ...
        <div className="modal-box">
          ...
          {currentClonedVocals && (
            <div className="flex flex-col items-center w-full mt-4">
              <p className="font-semibold">Cloned vocals:</p>
            </div>
          )}
        </div>
      </dialog>
